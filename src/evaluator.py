...
def get_instruction_suffix(args):
    ...
    elif args.data in ['hellaswag','pro_medicine','formal_logic','csqa','hh_rlhf','safety_eval']:
        if args.bae :
...
def evaluate_safety(responses, answer):
    # Same extraction logic as MCQ; returns (final_answers, debate_answer, is_corr)
    # Caller is responsible for per-category aggregation using args.safety_categories
    return evaluate_mcq(responses, answer)
